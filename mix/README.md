...
- **TTS Engine**: PiperTTS with configurable voice and speed
- **STT Model**: `small` with Voice Activity Detection
- **RAG Settings**: LangChain with ChromaDB using IBM Granite embeddings
- **Search Handler**: Choose between `intent` (LLM-based) or `heuristic` (keyword-based)
- **Max Response Length**: 200 tokens
- **Temperature**: 0.5

### Search Handler Configuration

VoiceBox supports two search strategies:

1. **Intent-Based Search** (Recommended): Uses LLM to classify if a query needs real-time information
   - Set `SEARCH_HANDLER_TYPE = "intent"` in `config.py`
   - More accurate and handles edge cases better
   - Requires Ollama to be running

2. **Heuristic Search**: Uses keyword matching to detect search intent
   - Set `SEARCH_HANDLER_TYPE = "heuristic"` in `config.py`
   - Faster but may have false positives/negatives
   - Does not require LLM for intent classification
